...
export function startScheduler(params) {
    ...
    const tick = async () => {
        ...
        try {
            ...
            for (let i = 0; i < 5; i += 1) {
                const job = await PostJobModel.findOneAndUpdate({ status: "READY", nextRunAt: { $lte: new Date() } }, { $set: { status: "POSTING" }, $inc: { attempts: 1 } }, { new: true }).lean();
                if (!job)
                    break;
...
async function processJob(params) {
    ...
    try {
        ...
        const usage = await DailyUsageModel.findOneAndUpdate({ userId: params.job.userId, accountId: params.job.accountId, dateKey: todayKey }, { $setOnInsert: { postsCount: 0 } }, { upsert: true, new: true }).lean();
        if (!usage || usage.postsCount >= settings.postsPerDayLimit) {
            ...
            await PostJobModel.updateOne({ _id: jobId }, { $set: { status: "SCHEDULED", nextRunAt: new Date(tomorrow.getTime() + jitterSeconds * 1000), lastError: "DAILY_LIMIT_REACHED" } });
            await writeAuditLog({
                env: params.env,
                userId: params.job.userId.toString(),
        ...
        if (params.job.targetType === "PAGE") {
            ...
            await PostJobModel.updateOne({ _id: jobId }, { $set: { status: "POSTED", postedAt: new Date(), fbPostId, lastError: undefined } });
            await writeAuditLog({
                env: params.env,
                userId: params.job.userId.toString(),
                action: "JOB_POST",
            ...
            await PostJobModel.updateOne({ _id: jobId }, { $set: { status: "POSTED", postedAt: new Date(), fbPostId: result.id, lastError: undefined } });
            await writeAuditLog({
                env: params.env,
                userId: params.job.userId.toString(),
                action: "JOB_POST",
        ...
        await DailyUsageModel.updateOne({ userId: params.job.userId, accountId: params.job.accountId, dateKey: todayKey }, { $inc: { postsCount: 1 } });
    }
    catch (err) {
...
async function refreshTokenIfNeeded(params) {
    ...
    try {
        ...
        await FacebookAccountModel.updateOne({ _id: params.accountId }, {
            $set: {
