...
export function jobsRoutes(params) {
    const router = Router();
    ...
    router.post("/:id/cancel", async (req, res, next) => {
        try {
            const authReq = req;
            const job = await PostJobModel.findOneAndUpdate({ _id: req.params.id, userId: authReq.auth.userId, status: { $in: ["SCHEDULED", "READY"] } }, { $set: { status: "CANCELLED" } }, { new: true }).lean();
            if (!job) {
    ...
    router.post("/:id/run-now", async (req, res, next) => {
        try {
            const authReq = req;
            const job = await PostJobModel.findOneAndUpdate({ _id: req.params.id, userId: authReq.auth.userId, status: { $in: ["SCHEDULED", "READY"] } }, { $set: { nextRunAt: new Date(), status: "READY" } }, { new: true }).lean();
            if (!job) {
