...
export function facebookRoutes(params: { env: Env }) {
  const router = Router();
  ...
  router.post("/connect/exchange", async (req, res, next) => {
    try {
      ...
      const account = await FacebookAccountModel.findOneAndUpdate(
        { userId: authReq.auth.userId, fbUserId: me.id },
        {
          $set: {
            name: me.name,
