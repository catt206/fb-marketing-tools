...
export function templatesRoutes(_params: { env: Env }) {
  const router = Router();
  ...
  router.put("/:id", async (req, res, next) => {
    try {
      ...
      if (body.imageUrl !== undefined) update.imageUrl = body.imageUrl ?? undefined;

      const template = await ContentTemplateModel.findOneAndUpdate(
        { _id: req.params.id, userId: authReq.auth.userId },
        { $set: update },
        { new: true }
      ).lean();
