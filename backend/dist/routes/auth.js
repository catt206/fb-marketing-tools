...
export function authRoutes(params) {
    ...
    router.post("/register", async (req, res, next) => {
        try {
            ...
            const token = sign({ userId: user._id.toString() }, params.env.JWT_SECRET, {
                expiresIn: params.env.JWT_EXPIRES_IN
            });
            await writeAuditLog({
                env: params.env,
    ...
    router.get("/me", requireAuth({ jwtSecret: params.env.JWT_SECRET }), async (req, res, next) => {
        try {
            const authReq = req;
